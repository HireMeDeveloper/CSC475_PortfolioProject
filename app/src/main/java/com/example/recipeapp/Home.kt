...
class Home : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = setUpHomeModels(inflater, container)
    ...
    private fun setUpHomeModels(inflater: LayoutInflater, container: ViewGroup?) : View?{
        val view = inflater.inflate(R.layout.fragment_home, container, false)
        val recyclerView : RecyclerView? = view?.findViewById(R.id.homeRecyclerView)

        // Get only meals from database
        if (dbHandler != null){
            val hints = dbHandler!!.getAllHints().shuffled()
            val trends = dbHandler!!.getReleventTrends()
            val recipes = dbHandler!!.getRecommendedRecipes()

            var cards = mutableSetOf<CardEntryModel>()

            cards.add(hints.random())
            cards.add(hints.random())
            cards.add(hints.random())
            ...
            cardEntryModels = cards.shuffled().toList() as ArrayList<CardEntryModel>
        } else {
            return view
        }
